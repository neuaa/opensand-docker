...
docker create -it --net lan-st4-${simulation_id} --ip $addr_ipv4 --ip6 $addr_ipv6 --privileged --hostname ws-st4-${index_workstation}-${simulation_id} --name ws-st4-${index_workstation}-${simulation_id} -v /home/neuaa/ProjetLong/simulation/opensand-docker/entites/WS/WS6/configuration_routes_${simulation_id}:/configuration_routes opensand-ws
