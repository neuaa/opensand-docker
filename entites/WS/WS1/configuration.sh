...
docker create -it --net lan-gw1-${simulation_id} --ip $addr_ipv4 --ip6 $addr_ipv6 --privileged --hostname ws-gw1-${index_workstation}-${simulation_id} --name ws-gw1-${index_workstation}-${simulation_id} -v $PWD/entites/WS/WS1/configuration_routes_${simulation_id}:/configuration_routes opensand-ws
