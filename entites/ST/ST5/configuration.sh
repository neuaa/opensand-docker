...
sudo sed -i 's+$lan_ipv6+'${st_lan_ipv6_addr}'/64+g' $PWD/entites/ST/ST5/daemonbck${simulation_id}.conf
...
docker create -it --privileged -v $PWD/entites/ST/ST5/daemonbck${simulation_id}.conf:/etc/opensand/daemonbck.conf --net opensand-network-${simulation_id} --ip $st_sat_addr --hostname st5-${simulation_id} --name st5-${simulation_id} opensand-daemon-2
...
docker exec st5-${simulation_id} "/start-daemon.sh"
