...
docker create -it --privileged -v $PWD/entites/ST/ST1/daemonbck${simulation_id}.conf:/etc/opensand/daemonbck.conf --net opensand-network-${simulation_id} --ip $st_sat_addr --hostname st1-${simulation_id} --name st1-${simulation_id} opensand-daemon
