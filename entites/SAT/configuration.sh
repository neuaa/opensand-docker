...
docker create -it --privileged -v $PWD/entites/SAT/daemonbck${simulation_id}.conf:/etc/opensand/daemonbck.conf --net opensand-network-${simulation_id} --ip $addr_ipv4 --hostname sat-${simulation_id} --name sat-${simulation_id} opensand-daemon-2
